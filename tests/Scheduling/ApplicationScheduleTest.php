...
class ApplicationScheduleTest extends TestCase {
    public function test_it_picks_up_tasks_registered_the_way_laravel_11_registers_them(): void {
        $this->registerOnStarting(fn (Schedule $schedule) => $schedule
            ->command('cron:late')
            ->everyMinutes(15, offsetBy: 7));

        $events = ApplicationSchedule::resolve($this->application())->events();
    ...
    public function test_resolving_the_container_binding_alone_would_have_missed_them(): void {
        ...
        $this->assertCount(0, $this->application()->make(Schedule::class)->events());
        $this->assertCount(1, ApplicationSchedule::resolve($this->application())->events());
    }

    public function test_it_returns_the_same_schedule_the_container_holds(): void {
        $this->assertSame(
            ApplicationSchedule::resolve($this->application()),
            $this->application()->make(Schedule::class),
        );
    }

    /** @param callable(Schedule): mixed $callback */
    private function registerOnStarting(callable $callback): void {
        ConsoleApplication::starting(function () use ($callback): void {
            $callback($this->application()->make(Schedule::class));
        });
    }
}
